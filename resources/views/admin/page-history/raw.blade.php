...
@section('content')
    <x-admin.card class="p-6 mb-6">
        ...
        <form method="get" action="{{ route('admin.page-history.raw') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A] bg-[#FDFDFC] dark:bg-[#0a0a0a]/50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">URL / Path</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Status</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Süre</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Asset</th>
                ...
                <tbody class="divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                    @forelse($logs as $log)
                        <tr class="hover:bg-[#FDFDFC] dark:hover:bg-[#0a0a0a]/30 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $log->visited_at?->format('d/m/Y H:i:s') ?? $log->created_at?->format('d/m/Y H:i:s') }}</td>
                            ...
                            <td class="px-6 py-4 text-sm max-w-xs truncate" title="{{ $log->full_url }}">{{ Str::limit($log->path, 50) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $log->status_code ?? '—' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $log->response_time_ms !== null ? number_format($log->response_time_ms) . ' ms' : '—' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $log->is_asset_request ? 'Evet' : 'Hayır' }}</td>
                            <td class="px-6 py-4 text-sm text-[#706f6c] dark:text-[#8F8F8B] max-w-xs truncate" title="{{ $log->user_agent }}">{{ Str::limit($log->user_agent, 40) }}</td>
                        </tr>
                    @empty
                        <tr>
