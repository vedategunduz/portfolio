...
@section('content')
    <x-admin.card class="p-6 mb-6">
        ...
        <form method="get" action="{{ route('admin.page-history.classified') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-[#e3e3e0] dark:border-[#3E3E3A] bg-[#FDFDFC] dark:bg-[#0a0a0a]/50">
                        ...
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Trafik tipi</th>
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Risk</th>
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Kural</th>
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] uppercase tracking-wider">Bot / Sebep</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                    @forelse($logs as $log)
                        <tr class="hover:bg-[#FDFDFC] dark:hover:bg-[#0a0a0a]/30 transition-colors">
                            <td class="px-4 lg:px-6 py-3 whitespace-nowrap text-sm text-[#1b1b18] dark:text-[#EDEDEC]">{{ $log->visited_at?->format('d/m/Y H:i:s') }}</td>
                            ...
                            <td class="px-4 lg:px-6 py-3 whitespace-nowrap text-sm">{{ $log->risk_level->label() }}</td>
                            ...
                            <td class="px-4 lg:px-6 py-3 text-sm text-[#706f6c] dark:text-[#8F8F8B] max-w-xs truncate" title="{{ $log->suspicion_reason ?? $log->bot_name }}">{{ Str::limit($log->suspicion_reason ?? $log->bot_name ?? '—', 35) }}</td>
                        </tr>
                    @empty
                        <tr>
            ...
            @forelse($logs as $log)
                ...
                <div class="p-4 hover:bg-[#FDFDFC] dark:hover:bg-[#0a0a0a]/30 transition-colors">
                    <div class="flex flex-wrap items-center gap-2 mb-2">
                        <span class="text-xs text-[#706f6c] dark:text-[#8F8F8B]">{{ $log->visited_at?->format('d/m/Y H:i') }}</span>
                        <span @class([
                            'px-2 py-0.5 text-xs font-medium rounded-sm',
                            'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' => $t->value === 'human',
                            'bg-[#706f6c]/15 text-[#706f6c] dark:text-[#8F8F8B]' => in_array($t->value, ['known_bot', 'monitoring', 'internal']),
                        ...
                        <span class="text-xs text-[#706f6c] dark:text-[#8F8F8B]">{{ $log->risk_level->label() }}</span>
                    </div>
                    <dl class="space-y-1 text-xs">
                        <div><dt class="inline text-[#706f6c] dark:text-[#8F8F8B]">IP:</dt> <dd class="inline"><a href="{{ route('admin.page-history.classified', ['ip' => $log->ip_address]) }}" class="text-[#D62113] hover:underline">{{ $log->ip_address }}</a></dd></div>
                        ...
                        @if($log->suspicion_reason ?? $log->bot_name)<div><dt class="inline text-[#706f6c] dark:text-[#8F8F8B]">Bot/Sebep:</dt> <dd class="inline break-all">{{ Str::limit($log->suspicion_reason ?? $log->bot_name ?? '—', 50) }}</dd></div>@endif
                    </dl>
                </div>
            @empty
