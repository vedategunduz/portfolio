...
@section('content')
    ...
    @if($errors->any())
        <div class="mb-6 rounded-sm border border-red-200 dark:border-red-900 bg-red-50/80 dark:bg-red-900/20 p-4">
            <p class="text-sm font-medium text-red-700 dark:text-red-300">{{ __('messages.profile.form_error') }}</p>
        </div>
    @endif
    ...
    <x-admin.card class="p-6 max-w-3xl">
        ...
        <form id="admin-profile-form" method="POST" action="{{ route('admin.profile.update') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label for="name" class="block text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] mb-1">{{ __('messages.profile.name_label') }}</label>
                ...
                <div>
                    <label for="email" class="block text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] mb-1">{{ __('messages.profile.email_label') }}</label>
            ...
            <div class="pt-6 border-t border-[#e3e3e0] dark:border-[#3E3E3A]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <label for="current_password" class="block text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] mb-1">{{ __('messages.profile.current_password') }}</label>
                    ...
                    <div>
                        <label for="password" class="block text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] mb-1">{{ __('messages.profile.new_password') }}</label>
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-medium text-[#706f6c] dark:text-[#8F8F8B] mb-1">{{ __('messages.profile.confirm_password') }}</label>
