...
class GetBlogAnalyticsOverviewDataAction
{
    public function execute($from, $to, bool $includeBots = false, ?int $postId = null): array
    {
        ...
        $rawBotViews = (int) DB::table('analytics_post_views as pv')
            ...
            ->when($postId, fn ($query) => $query->where('pv.post_id', $postId))
            ->count();
        ...
        $referrerDomainExpression = $this->referrerDomainExpression();
        $sourceBase = (clone $baseViews)
            ->selectRaw("
                ...
                {$referrerDomainExpression} as referrer_domain
            ");
    ...
    private function buildReconciledTopRows(Collection $rows, int $expectedTotalViews, bool $hasEngagedReads = true): Collection
    {
    ...
    private function referrerDomainExpression(): string
    {
        ...
        $normalized = "REPLACE(REPLACE(s.referrer, 'https://', ''), 'http://', '')";
        ...
        return "
            COALESCE(
                NULLIF(SUBSTRING_INDEX({$normalized}, '/', 1), ''),
                'direct'
            )
        ";
    }
